...
SPIClass spiSD(HSPI);  // use HSPI bus for SD, leaves VSPI free
#define IR_SLOTS 8
...
enum DeauthMode { DEAUTH_SCANNER, DEAUTH_TRANSMITTER, DEAUTH_BEACON };
...
// SD Tools
char sdFiles[20][32];  // fixed char arrays, no heap allocation
uint8_t sdFileCount = 0;
int8_t sdFileSel = 0;
bool sdFormatConfirm = false;  // confirm before format
int8_t menuSel = 0;
...
// ======================================================
void startWifiScan() {
  // ensure radio is fully reset before starting a new async scan —
  // calling scanNetworks too soon after a previous scan can hang
  WiFi.scanDelete();
  wifiState=WIFI_SCANNING; netCount=0; netScroll=0; scanAnim=0;
  scanStartMs=millis(); scanAnimTimer=millis();
  WiFi.mode(WIFI_STA); WiFi.setSleep(false);
  delay(50); // let radio settle after mode change before scanning
...
void updateWifiScan() {
  ...
  if(n==WIFI_SCAN_RUNNING) return;          // still scanning, keep waiting

  if(n==WIFI_SCAN_FAILED){
    // scan failed to start/complete — retry once instead of hanging
    WiFi.scanDelete();
    WiFi.scanNetworks(true);
    scanStartMs=millis(); // reset timeout window
    return;
  }

  // safety timeout — if scan takes over 10s, bail out to results (empty)
  if(millis()-scanStartMs > 10000){
    netCount=0;
    WiFi.scanDelete();
    wifiState=WIFI_RESULTS;
    beepScan();
    return;
  }
...
void drawMenuIcon(uint8_t idx, uint8_t tx, uint8_t TY, uint8_t TW){
  ...
  if(idx==0){
    ...
    // SD tools: microSD card icon — body with notched corner + contact lines
    // card outline with angled top-left corner (classic SD card shape)
    u8g2.drawLine (cx-6, iy+3, cx-3, iy);     // angled corner
    u8g2.drawLine (cx-3, iy,   cx+6, iy);     // top edge
    u8g2.drawLine (cx+6, iy,   cx+6, iy+16);  // right edge
    u8g2.drawLine (cx+6, iy+16,cx-6, iy+16);  // bottom edge
    u8g2.drawLine (cx-6, iy+16,cx-6, iy+3);   // left edge
    // gold contact pins at bottom of card
    u8g2.drawBox(cx-4, iy+11, 2, 4);
    u8g2.drawBox(cx-1, iy+11, 2, 4);
    u8g2.drawBox(cx+2, iy+11, 2, 4);
  }
}
...
uint16_t  attackPeakRate = 0;
uint32_t  attackFrameCount = 0;
bool      attackOngoing = false;
unsigned long attackStartMs = 0;
unsigned long attackDurationMs = 0;
bool      showLog = false;
...
void startBeaconSpam() {
  ...
  deauthRunning = true;   // ← Important
  Serial.println("Advanced Beacon Spam Started");
}
void stopBeaconSpam() {
  beaconSpamRunning = false;
  Serial.printf("Beacon Spam Stopped - %u beacons sent\n", beaconCount);
}
...
void updateBeaconSpam() {
  ...
  while (i < ssidsLen) {
    ...
    for (int k = 0; k < 3; k++) {
      esp_wifi_80211_tx(WIFI_IF_STA, beaconPacket, 100, true);
      delay(1);
    }
...
void updateDeauth() {
  ...
  if (deauthMode == DEAUTH_SCANNER) {
    // Scanner logic (your original code)
  ...
  else if (deauthMode == DEAUTH_TRANSMITTER) {
    // Transmitter logic
    ...
    for (int i = 0; i < txAttackerCount; i++) {
      ...
      for (int b = 0; b < 5; b++) {
        ...
        delay(2);
           ...
           else if (deauthMode == DEAUTH_BEACON) {
          updateBeaconSpam();
          // UI
          u8g2.clearBuffer();
          u8g2.drawBox(0,0,128,13);
          u8g2.setDrawColor(0);
          u8g2.setFont(u8g2_font_5x7_tr);
          u8g2.drawStr(3,10,"BEACON SPAM");
          u8g2.setDrawColor(1);
          u8g2.setFont(u8g2_font_6x10_tr);
          u8g2.drawStr(15,35,"Flooding...");
          char buf[20];
          sprintf(buf,"Sent: %lu", beaconCount);
          u8g2.setFont(u8g2_font_4x6_tr);
          u8g2.drawStr(2,50,buf);
          u8g2.drawStr(2,63,"LONG LEFT: stop");
          u8g2.sendBuffer();
        }
}
...
void drawDeauthSubmenu() {
  ...
  u8g2.setFont(u8g2_font_6x10_tr);
  u8g2.drawStr(15, 28, deauthMode == DEAUTH_SCANNER     ? "→ SCANNER"     : "  SCANNER");
  u8g2.drawStr(15, 40, deauthMode == DEAUTH_TRANSMITTER ? "→ TRANSMITTER" : "  TRANSMITTER");
  u8g2.drawStr(15, 52, deauthMode == DEAUTH_BEACON      ? "→ BEACON SPAM" : "  BEACON SPAM");
...
bool formatSDCard() {
  ...
  // SD.format() doesn't exist in ESP32 SD lib — delete all files manually
  File root = SD.open("/");
  if (!root) return false;
  File file = root.openNextFile();
  while (file) {
    char fname[34]; snprintf(fname, 34, "/%s", file.name());
    file.close();
    SD.remove(fname);
...
void listSDFiles() {
  ...
  while (file && sdFileCount < 20) {
    if(!file.isDirectory()){
      strncpy(sdFiles[sdFileCount], file.name(), 31);
      sdFiles[sdFileCount][31] = '\0';
      sdFileCount++;
    }
    file = root.openNextFile();
  }
  root.close();
  if(sdFileSel >= sdFileCount) sdFileSel = max(0, sdFileCount-1);
}

void deleteSelectedFile() {
  if (sdFileCount == 0 || sdFileSel >= sdFileCount) return;
  char path[34]; snprintf(path, 34, "/%s", sdFiles[sdFileSel]);
  if (SD.remove(path)) {
    listSDFiles();
  }
}
...
void saveIRToSD() {
  if (!sdMounted) return;
  // remove old file first so we overwrite, not append
  SD.remove("/ircodes.txt");
  File file = SD.open("/ircodes.txt", FILE_WRITE);
  if (!file) return;
  for (int i = 0; i < IR_SLOTS; i++) {
    // always write all slots so load can use index
    file.printf("SLOT:%d\n", i);
    file.printf("LABEL:%s\n", irCodes[i].label);
    file.printf("HAS:%d\n", irCodes[i].hasCode ? 1 : 0);
    if (irCodes[i].hasCode) {
      file.printf("RAW:%d\n", irCodes[i].isRaw ? 1 : 0);
      if (irCodes[i].isRaw) {
        file.printf("LEN:%d\n", irCodes[i].rawLen);
        for (uint16_t j = 0; j < irCodes[i].rawLen; j++) {
          file.printf("%u", irCodes[i].rawBuf[j]);
          if (j < irCodes[i].rawLen - 1) file.print(",");
        }
        file.println();
      } else {
        file.printf("PROTO:%d\n", (int)irCodes[i].protocol);
        file.printf("CODE:%llu\n", irCodes[i].code);
        file.printf("BITS:%d\n", irCodes[i].bits);
      }
    }
  }
...
void loadIRFromSD() {
  if (!sdMounted) return;
  File file = SD.open("/ircodes.txt", FILE_READ);
  if (!file) return;
  int slot = -1;
  char lineBuf[64];
  while (file.available()) {
    ...
    if (len == 0) continue;
    // parse
    if (strncmp(lineBuf, "SLOT:", 5) == 0) {
      slot = atoi(lineBuf + 5);
    } else if (slot >= 0 && slot < IR_SLOTS) {
      if (strncmp(lineBuf, "LABEL:", 6) == 0) {
        strncpy(irCodes[slot].label, lineBuf + 6, 11);
        irCodes[slot].label[11] = '\0';
      } else if (strncmp(lineBuf, "HAS:", 4) == 0) {
        irCodes[slot].hasCode = atoi(lineBuf + 4);
      } else if (strncmp(lineBuf, "RAW:", 4) == 0) {
        irCodes[slot].isRaw = atoi(lineBuf + 4);
      } else if (strncmp(lineBuf, "LEN:", 4) == 0) {
        irCodes[slot].rawLen = atoi(lineBuf + 4);
      } else if (strncmp(lineBuf, "PROTO:", 6) == 0) {
        irCodes[slot].protocol = (decode_type_t)atoi(lineBuf + 6);
      } else if (strncmp(lineBuf, "CODE:", 5) == 0) {
        irCodes[slot].code = strtoull(lineBuf + 5, nullptr, 10);
      } else if (strncmp(lineBuf, "BITS:", 5) == 0) {
        irCodes[slot].bits = atoi(lineBuf + 5);
      } else if (irCodes[slot].isRaw && irCodes[slot].rawLen > 0) {
        // parse comma-separated raw timings
        char* ptr = lineBuf;
        uint16_t j = 0;
        while (*ptr && j < irCodes[slot].rawLen && j < IR_RAW_BUF) {
          irCodes[slot].rawBuf[j++] = atoi(ptr);
          ptr = strchr(ptr, ',');
          if (!ptr) break;
          ptr++;
        }
...
void drawSDTools() {
  ...
  u8g2.setDrawColor(0);
  u8g2.setFont(u8g2_font_5x7_tr);
  // show mount status in header
  char hbuf[22];
  snprintf(hbuf,22,"SD TOOLS %s", sdMounted?"[OK]":"[NO CARD]");
  u8g2.drawStr(3,10,hbuf);
  u8g2.setDrawColor(1);

  if(sdFormatConfirm){
    // format confirmation screen
    u8g2.setFont(u8g2_font_6x10_tr);
    u8g2.drawStr(14,28,"Format card?");
    u8g2.setFont(u8g2_font_5x7_tr);
    u8g2.drawStr(8,42,"ALL data will be lost!");
    u8g2.drawHLine(0,57,128);
    u8g2.setFont(u8g2_font_4x6_tr);
    u8g2.drawStr(2,63,"SEL:confirm  any other:cancel");
    u8g2.sendBuffer(); return;
  }

  if (!sdMounted) {
    u8g2.setFont(u8g2_font_5x7_tr);
    u8g2.drawStr(14,35,"No SD card found");
    u8g2.setFont(u8g2_font_4x6_tr);
    u8g2.drawStr(10,50,"Check wiring & reboot");
    u8g2.drawHLine(0,57,128);
    u8g2.drawStr(2,63,"LEFT:back");
    u8g2.sendBuffer(); return;
  }

  if (sdFileCount == 0) {
    u8g2.setFont(u8g2_font_5x7_tr);
    u8g2.drawStr(20,35,"Card is empty");
  } else {
    for (uint8_t i = 0; i < 4; i++) {
      ...
      uint8_t y = 22 + i*11;
      if (i == 0) {
        u8g2.drawRBox(0, y-8, 122, 11, 2);
        u8g2.setDrawColor(0);
      }
      u8g2.setFont(u8g2_font_5x7_tr);
      u8g2.drawStr(4, y, sdFiles[idx]);
      u8g2.setDrawColor(1);
    }
    // scroll dots
    if(sdFileCount>4){
      for(int i=0;i<sdFileCount;i++){
        if(i==sdFileSel) u8g2.drawDisc(125,15+i*4,1);
        else              u8g2.drawPixel(125,15+i*4);
      }
    }
  }
  ...
  u8g2.drawHLine(0,57,128);
  u8g2.setFont(u8g2_font_4x6_tr);
  u8g2.drawStr(2,63,"U/D:pick SEL:del R:fmt IR:save L:back");
  u8g2.sendBuffer();
}
...
void handleSDInput() {
  if(sdFormatConfirm){
    if(btnPressed(IDX_SEL)){
      sdFormatConfirm=false;
      formatSDCard();
      listSDFiles();
    } else if(btnPressed(IDX_UP)||btnPressed(IDX_DOWN)||
              btnPressed(IDX_LEFT)||btnPressed(IDX_RIGHT)){
      sdFormatConfirm=false; // any other key cancels
    }
    return;
  }

  if(btnPressed(IDX_LEFT)){ appState=STATE_MENU; return; }
  ...
  // RIGHT = enter format confirmation (not immediate)
  if(btnPressed(IDX_RIGHT)){
    sdFormatConfirm=true;
  }

  // UP+SEL held together = save IR codes — use separate long-press SEL
  // simpler: dedicated long-press SELECT = save IR
  static unsigned long sdSelHold=0;
  static bool sdSelFired=false;
  if(btnHeld(IDX_SEL)){
    if(sdSelHold==0) sdSelHold=millis();
    if(!sdSelFired && millis()-sdSelHold>800){
      sdSelFired=true;
      saveIRToSD();
      listSDFiles();
    }
  } else { sdSelHold=0; sdSelFired=false; }
}
// ======================================================
// DRAW — DEAUTH (Scanner)
// ======================================================
...
void handleDeauthInput(){
  if (inDeauthSubmenu) {
    // Navigation
    if (btnPressed(IDX_UP)) {
      deauthMode = (DeauthMode)((deauthMode + 2) % 3);  // cycle backward
    }
    if (btnPressed(IDX_DOWN) || btnPressed(IDX_RIGHT)) {
      deauthMode = (DeauthMode)((deauthMode + 1) % 3);  // cycle forward
    }
    ...
    if (btnPressed(IDX_SEL)) {
      ...
      Serial.printf("Starting mode: %d\n", deauthMode);  // debug
      
      if (deauthMode == DEAUTH_SCANNER) {
        startDeauthScanner();
      } else if (deauthMode == DEAUTH_TRANSMITTER) {
        startDeauthTransmitter();
      } else if (deauthMode == DEAUTH_BEACON) {
        startBeaconSpam();
      }
    }
  ...
  if (irRecv.decode(&irResults)) {
    ...
    if (irResults.decode_type != UNKNOWN && irResults.decode_type != RAW) {
      ...
      for (uint16_t i = 0; i < len; i++) {
        irCodes[irSel].rawBuf[i] = irResults.rawbuf[i+1]; // already in microseconds
      }
...
void stopIR(){
  irRecv.disableIRIn();
  if(sdMounted) saveIRToSD(); // auto-save on exit
}
// ======================================================
    ...
    case STATE_MOUSE:
      ...
      if (inDeauthSubmenu) {
        ...
        if (deauthMode == DEAUTH_SCANNER) {
          ...
          u8g2.setFont(u8g2_font_6x10_tr);
          u8g2.drawStr(15,32,"Flooding fake APs...");
